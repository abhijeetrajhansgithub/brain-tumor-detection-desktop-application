...
class MainPage_13(QMainWindow):
    def __init__(self, username):
        ...
        self.setWindowTitle("Main Page v5.0.0")
    ...
    def fShow_Diagnose_Frame(self):
        ...
        from Image.load_image import return_xai_button_bg_image_path
        xai_button_bg_image_path = return_xai_button_bg_image_path()
        xai_button_bg_image_path = xai_button_bg_image_path.replace("\\", "/")

        print("xai_button_bg_image_path:", xai_button_bg_image_path)

        # xAI Button (Initially Hidden)
        self.xai_button_for_diagnosis = QPushButton("xAI")
        # self.xai_button_for_diagnosis.setStyleSheet(f"""
        #     QPushButton {{
        #         background-image: url('{xai_button_bg_image_path}');
        ...
        self.xai_button_for_diagnosis.setStyleSheet(self.fGet_Button_Style())
        self.xai_button_for_diagnosis.clicked.connect(self.fGet_xAI_Results)
        self.xai_button_for_diagnosis.setVisible(False)
        ...
        diagnosis_content_layout.addWidget(self.xai_button_for_diagnosis)
    ...
    def fGet_xAI_Results(self):
        from PyQt5.QtWidgets import QDialog, QLabel, QVBoxLayout, QPushButton
        from PyQt5.QtGui import QPixmap
        from Models.XAI.use_xai_for_pixmap import classify_and_explain_pixmap
        from Models.ModelFunctionsRepo.model_prep_functions import LOAD_MODELS_for_xAI

        print("Getting xAI Results")

        self.H5_MODEL_xAI, self.KERAS_MODEL_xAI = LOAD_MODELS_for_xAI()

        if self.IMAGE_FOR_DIAGNOSIS is None:
            print("Error: No image loaded for diagnosis.")
            return

        if isinstance(self.IMAGE_FOR_DIAGNOSIS, QPixmap):
            image_for_xai = self.IMAGE_FOR_DIAGNOSIS
        else:
            print("Error: IMAGE_FOR_DIAGNOSIS is not a QPixmap.")
            return

        overlaid_image, predicted_class, score = classify_and_explain_pixmap(
            h5_model=self.H5_MODEL_xAI,
            keras_model=self.KERAS_MODEL_xAI,
            pixmap=image_for_xai
        )

        if overlaid_image is None:
            print("Error: classify_and_explain_pixmap failed.")
            return

        # Mapping predictions to class labels
        map_ = {0: 'no_tumor', 1: 'glioma_tumor', 2: 'meningioma_tumor', 3: 'pituitary_tumor'}
        predicted_label = map_.get(predicted_class, "Unknown")
        ...
        dialog = QDialog(self)
        dialog.setWindowTitle("Classification Result")

        layout = QVBoxLayout()

        # Image label
        image_label = QLabel()
        image_label.setPixmap(overlaid_image)
        image_label.setScaledContents(True)

        # Prediction label
        prediction_label = QLabel(f"Predicted Class: {predicted_label}\nConfidence Score: {score:.8f}")
    ...
    def fClassify_Tumor(self, image):
        ...
        if self.output_label_for_diagnosis.text():
            ...
            self.xai_button_for_diagnosis.setVisible(True)
        # Notify
    ...
    def fDisplay_Patient_Record(self, patient_record):
        from Main.utils import get_current_system_path, change_path_to_db
        from API.DatabaseAPI.client_database_connection import get_tumor_diagnosis_details
        ...
        tumor_diagnosis = get_tumor_diagnosis_details(patient_id=patient_id, PATH_=dbPATH_)
        print("TUMOR DIAGNOSIS: ", tumor_diagnosis)

        if not tumor_diagnosis:
            QMessageBox.warning(self, "Error", "No tumor diagnosis found for this patient.")
            return
        ...
        to_pdf_button.clicked.connect(lambda: self.fTo_PDF(patient_record=patient_record, tumor_diagnosis=tumor_diagnosis))

        # generate report button
        generate_report_button = QPushButton("Generate Report")
        generate_report_button.clicked.connect(lambda: self.fGenerate_Report(patient_record=patient_record, tumor_diagnosis=tumor_diagnosis))
    ...
    @staticmethod
    def fTo_PDF(patient_record, tumor_diagnosis):
        """Generate a PDF file with patient details and image."""
        file_path, _ = QFileDialog.getSaveFileName(None, "Save PDF", f"{patient_record['first_name']}_{patient_record['last_name']}__ pid = {patient_record['patient_id']} _tumor_report.pdf", "PDF Files (*.pdf)")
        if not file_path:
            return
        ...
        details = (
            ...
            f"Phone: {patient_record['phone_number']}\n"
            f"Email: {patient_record['email']}\n"
            f"Tumor Diagnosis: {tumor_diagnosis[0][1]}\n"
            f"Tumor Diagnosed At: {tumor_diagnosis[0][2]}\n"
        )
    ...
    @staticmethod
    def fGenerate_Report(patient_record, tumor_diagnosis):
        """Generate a text report with patient details."""
        file_path, _ = QFileDialog.getSaveFileName(None, "Save Report", f"{patient_record['first_name']}_{patient_record['last_name']}__ pid = {patient_record['patient_id']} _tumor_report.txt", "Text Files (*.txt)")
        if not file_path:
            return
