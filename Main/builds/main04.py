from PyQt5.QtWidgets import (
    QApplication, QMainWindow, QWidget, QVBoxLayout, QHBoxLayout, QLineEdit, QPushButton, QStackedWidget
)
...
class MainWindow(QMainWindow):
    ...
    def create_signup_page(self):
        ...
        self.signup_user_email_input.returnPressed.connect(self.signup_password_input.setFocus)
    ...
    def override_keyPressEvent(self, widget):
        ...
        def custom_keyPressEvent(event):
            if event.key() == Qt.Key_Up:
                if widget == self.signup_user_email_input:
    ...
    def signup(self):
        from API.DatabaseAPI.client_database_connection import register_user, check_username_uniqueness, check_email_uniqueness
        ...
        if len(username) == 0 or len(email) == 0 or len(password) == 0 or len(confirm_password) == 0:
            ...
            if check_username_uniqueness(username, dbPATH_) and check_email_uniqueness(email, dbPATH_) and password == confirm_password:
                register_user(username, email, password, dbPATH_)
