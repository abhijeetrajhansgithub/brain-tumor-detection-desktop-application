...
def process_images_array(image):
    """Processes an OpenCV image array instead of a file path."""
...
def process_and_display(image):
    """Process the image and display contrast variations, segmentation, and color buckets."""
    contrast_images = enhance_contrast(image)
    ...
    for i in range(min(24, len(contrast_images))):  # Ensure we don't go out of bounds
        row, col = divmod(i, 3)
        contrast_image = contrast_images[i]
        segmented = segment_and_highlight(contrast_image)
        color_buckets = apply_color_buckets(segmented)

        axes[row, col].imshow(cv2.cvtColor(color_buckets, cv2.COLOR_BGR2RGB))
        axes[row, col].set_title(f"Contrast Level {i + 1} (Bucketed)")
        axes[row, col].axis("off")

    plt.show()


def cv2_get_contrasting_color_images(qpixmap_image: QPixmap):
    """Converts QPixmap to OpenCV image and processes it to generate contrast images."""
    qimage = qpixmap_image.toImage()
    qimage = qimage.convertToFormat(QImage.Format_RGB888)  # Ensure RGB format

    width, height = qimage.width(), qimage.height()
    buffer = qimage.bits().asstring(qimage.byteCount())  # Get raw image data

    image = np.frombuffer(buffer, dtype=np.uint8).reshape((height, width, 3))  # Convert to NumPy array
    image = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)  # Convert RGB to BGR for OpenCV compatibility

    return process_images_array(image)
